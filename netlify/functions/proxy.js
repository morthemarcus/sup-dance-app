...
const paidStyle = { font: { bold: true, color: { rgb: '2A7A00' }, sz: 11 }, fill: { fgColor: { rgb: 'E6F4E0' }, patternType: 'solid' }, alignment: { horizontal: 'center' } };
const unpaidStyle = { font: { bold: true, color: { rgb: 'C00000' }, sz: 11 }, fill: { fgColor: { rgb: 'FDE8E8' }, patternType: 'solid' }, alignment: { horizontal: 'center' } };
const partialStyle = { font: { bold: true, color: { rgb: '856404' }, sz: 11 }, fill: { fgColor: { rgb: 'FFF3CD' }, patternType: 'solid' }, alignment: { horizontal: 'center' } };

function pStyle(s) { return s === 'paid' ? paidStyle : s === 'partial' ? partialStyle : unpaidStyle; }
function pLabel(s) { return s === 'paid' ? 'Paid' : s === 'partial' ? 'Partial' : 'Unpaid'; }

function styleSheet(ws, styleFn) {
  if (!ws['!ref']) return;
...
function buildXlsx(data) {
  ...
  styleSheet(sWs, (r) => r===0 ? headerStyle : r%2===0 ? altStyle : cellStyle);
  ...
  styleSheet(cWs, (r) => r===0 ? headerStyle : cGroupRows.has(r) ? groupStyle : r%2===0 ? altStyle : cellStyle);
  ...
  months.forEach(mk => {
    ...
    const mRows = [headers];
    const mGroupRows = new Set();
    const mPayRows = {};

    monthCls.forEach(cls => {
      const cls_students = alpha(activeStudents.filter(s=>(s.assignedClasses||[]).includes(cls.id)));
      if (!cls_students.length) return;
      mGroupRows.add(mRows.length);
      ...
      cls_students.forEach(s => {
        ...
        mPayRows[mRows.length] = status;
        mRows.push([s.name, s.phone||'', ...cells, total||'', total?due:'', pLabel(status), mnotes]);
      });
    });
    ...
    styleSheet(mWs, (r,c) => {
      ...
      if (mGroupRows.has(r)) return groupStyle;
      if (c===payCol && mPayRows[r]) return pStyle(mPayRows[r]);
  ...
  styleSheet(nWs, (r)=>r===0?headerStyle:r%2===0?altStyle:cellStyle);
